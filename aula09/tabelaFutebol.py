...
def main():
    ...
    #tab = sorted(tabela , key= lambda tabela: (pontos(tabela) ,  tabela[4]-tabela[5]) )
    #
    tab = sorted(tabela, key= lambda tabela:pontos(tabela), reverse=True)     # sort on secondary key
    sorted(tab, key=lambda tabela:tabela[4]-tabela[5] )
    printTabela(tab)
